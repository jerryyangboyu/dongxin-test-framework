...
public class OrderedPackageLoader extends AbstractPackageLoader{
	 ...
	 */
	static class OutTimeLimitException extends Exception {
	...
	/*
	* description：初始化Service类，当前Service类不能自动注入任何依赖
	* author：zhangying
	* date:2021/08/13
	* */
	private void processService(Class<?> clazz) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, InstantiationException {
	...
	private void processTestInit(Method m,Object target,String className){
		logger.info("Test Class {} is initting",
				className);
		try {
	...
	protected void processTestClazz(Class<?> clazz,Object target) throws InvocationTargetException, IllegalAccessException {
	...
	protected Object[] dependencyDetection(Method m, Object target) throws InvocationTargetException, IllegalAccessException {
		List<Object> objects=new LinkedList<>();
		for(Parameter parameter:m.getParameters()){
			Class<?> clazz=parameter.getClass();
			objects.add(this.serviceClassInstanceMap.containsKey(clazz)?this.serviceClassInstanceMap.get(clazz):null);
		}
